...
import { buildGraph, loadEmpireConfig, loadFalconConfig, loadRoutes } from "@falcon/core";
import type { FastifyInstance } from "fastify";
import { afterEach, describe, expect, it } from "vitest";
import { buildApp, type ErrorResponse, type OddsResponse, type UniverseResponse } from "./app.js";
...
/** Boots the app on a fixture universe and registers it for teardown. */
async function startApp(fixture: string): Promise<FastifyInstance> {
  const falconConfig = await loadFalconConfig(join(examplesDir, fixture, "millennium-falcon.json"));
  const routes = loadRoutes(falconConfig.routesDb);
  const graph = buildGraph(routes, [falconConfig.departure, falconConfig.arrival]);
  openApp = await buildApp({ falconConfig, graph, logger: false });
  return openApp;
}
...
describe("app", () => {
  ...
  it("GET /api/universe returns the fixture's planets, routes and startup mission fields", async () => {
    const app = await startApp("example2");

    const response = await app.inject({ method: "GET", url: "/api/universe" });

    expect(response.statusCode).toBe(200);
    const body = response.json<UniverseResponse>();
    ...
    expect(again.json<UniverseResponse>().routes).toEqual(body.routes);
  });
  ...
  it("POST /api/odds computes odds from a JSON body, matching example2/answer.json", async () => {
    ...
    expect(response.statusCode).toBe(200);
    const body = response.json<OddsResponse>();
  ...
  it("POST /api/odds accepts a real multipart upload, matching example3/answer.json", async () => {
    const app = await startApp("example3");
  ...
  it("POST /api/odds returns 0% when the destination is unreachable in time (example1)", async () => {
    const app = await startApp("example1");
  ...
  it("collapses duplicate {planet, day} sightings in the echoed bountyHunters", async () => {
    ...
    expect(response.statusCode).toBe(200);
    expect(response.json<OddsResponse>().bountyHunters).toEqual([
      { planet: "Hoth", day: 6 },
      { planet: "Hoth", day: 7 },
    ]);
  });
  ...
  it("POST /api/odds returns 400 for a malformed empire.json payload", async () => {
    ...
    expect(response.json<ErrorResponse>().error).toMatch(/countdown/);
  });
});
