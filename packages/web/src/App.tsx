...
import { ApiError, fetchOdds, fetchUniverse } from "./api/client";
...
export default function App(): React.JSX.Element {
  const universe = useQuery({ queryKey: ["universe"], queryFn: fetchUniverse });
  ...
  return (
    <main className="mx-auto flex max-w-6xl flex-col gap-10 px-6 pt-16 pb-12">
        ...
        <section className="text-center text-dim">
          {universe.data && (
            <p className="flex items-center justify-center gap-2">
              ...
              <span>
                Departing <strong>{universe.data.departure}</strong> for <strong>{universe.data.arrival}</strong>, autonomy{" "}
                <strong>{universe.data.autonomy}</strong> day{universe.data.autonomy === 1 ? "" : "s"}.
              </span>
            </p>
          )}
          {universe.isError && (
            <p className="flex items-start justify-center gap-2 text-alert">
              ...
              {universe.error instanceof ApiError ? universe.error.message : "Could not reach the onboard computer."}
            </p>
          )}
        </section>
      ...
      {universe.isError && (
        <div
          ...
          className="flex flex-col items-center gap-2 rounded-xl border border-seam bg-panel/80 p-8 text-center text-alert"
        >
      ...
      {universe.data && (
        <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_18rem]">
          <div className={`rounded-xl border border-seam bg-panel/80 p-4 transition-opacity ${odds.isPending ? "opacity-50" : ""}`}>
            <StarMap universe={universe.data} itinerary={odds.data?.itinerary ?? null} sightings={odds.data?.bountyHunters ?? []} />
          </div>
          <MapDetails universe={universe.data} result={odds.data} />
        </section>
      )}
      {universe.isLoading && <div className="h-64 animate-pulse rounded-xl border border-seam bg-panel/40" />}
    </main>
  );
}
